...
class MyProj:
    def payandconfirm(self):
            ...
            if (not result[0][5] and not result[0][4]):
                #1 seat booked
                print("1 seat")
                print(result[0][2],result[0][3],self.text1.get(),self.text4.get())
                c.execute('INSERT INTO Travelling_passengers (ticketno,seatno,name,age) VALUES(?,?,?,?)',[(result[0][0]),(result[0][3]),(self.text1.get()),(self.text4.get())])
                db.commit()
            elif(not result[0][5]):
                #2 seats booked
                print("2 seats")
                print(result[0][2],result[0][4],self.text1.get(),self.text4.get())
                print(result[0][2],result[0][3],self.text2.get(),self.text4.get())
                c.execute('INSERT INTO Travelling_passengers (ticketno,seatno,name,age) VALUES(?,?,?,?)',[(result[0][0]),(result[0][4]),(self.text1.get()),(self.text4.get())])
                c.execute('INSERT INTO Travelling_passengers (ticketno,seatno,name,age) VALUES(?,?,?,?)',[(result[0][0]),(result[0][3]),(self.text2.get()),(self.text5.get())])
                db.commit()
            else:
                #3 seats booked
                print("3 seats")
                print(result[0][2],result[0][5],self.text1.get(),self.text4.get())
                print(result[0][2],result[0][4],self.text2.get(),self.text5.get())
                print(result[0][2],result[0][3],self.text3.get(),self.text6.get())
                c.execute('INSERT INTO Travelling_passengers (ticketno,seatno,name,age) VALUES(?,?,?,?)',[(result[0][0]),(result[0][5]),(self.text1.get()),(self.text4.get())])
                c.execute('INSERT INTO Travelling_passengers (ticketno,seatno,name,age) VALUES(?,?,?,?)',[(result[0][0]),(result[0][4]),(self.text2.get()),(self.text5.get())])
                c.execute('INSERT INTO Travelling_passengers (ticketno,seatno,name,age) VALUES(?,?,?,?)',[(result[0][0]),(result[0][3]),(self.text3.get()),(self.text6.get())])
                db.commit()
            root1.destroy()
            os.system('sample41.py')
            '''self.frame1 = Frame(root1, bg="dark slate blue").grid()
            self.labl1 = Label(self.frame1, text="                             E-TICKET BOOKING-IRCTC                   ", pady=5, font=("Trebuchet MS", 35), bg="dark slate blue",fg="goldenrod")
            self.labl1.grid(row=0,column=1, columnspan=20)
            self.labl2 = Label(self.frame1, text="           BOOKING STATUS", pady=5, font=("Trebuchet MS", 20), bg="dark slate blue", fg="gold",padx=20)
            self.labl2.grid(row=1, column=1, columnspan=20)
            self.labl7 = Label(self.frame1, text="                       ----------------------------------------------------------------------------------", pady=5, font=("Trebuchet MS", 20), bg="dark slate blue", fg="gold",
                 padx=20)
            self.labl7.grid(row=2, column=0, columnspan=20)
            self.labl3 = Label(self.frame1, text="                          Booked Ticket Details:", fg="goldenrod", pady=5, bg="dark slate blue", font=("Trebuchet MS", 18))
            self.labl3.grid(row=10,column=12,sticky=W)
            self.txt1 = Text(self.frame1, font=('Trebuchet MS', 12, 'bold'),bd=4,bg='white',width=64,height=12)
            self.txt1.grid(row=11,column=12)
            self.labl4 = Label(self.frame1, text="\nTICKET CONFIRMED!!", pady=5, font=("Trebuchet MS", 30), bg="dark slate blue",fg="gold")
            self.labl4.grid(row=25,column=12)
            self.but1=Button(self.frame1, text='DONE',fg="blue",activebackground="goldenrod",font=("Trebuchet MS", 14),activeforeground="gold", bg="gold", height=1, width=8)
            self.but1.grid(row=27,column=12)'''
    ...
    def __init__(self,master):
            ...
            w11 = OptionMenu(self.frame, variable, *OPTIONS1)
            ...
            w21 = OptionMenu(self.frame, variable2, *OPTIONS1)
            ...
            w31 = OptionMenu(self.frame, variable3, *OPTIONS1)
